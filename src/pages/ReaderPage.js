...
export default class ReadPage extends Component {
  ...
  getArticle(type, params) {
    DaoArticle.getArticle(type, params)
      .then(result => {
        this.setState({
          article: result.data,
        });
  ...
  async initLiveState() {
    if (this.state.article) {
      this.setState({
        hasLive:
          (await DaoArticle.hasArticleCollected(this.state.article)) != -1,
      });
    }
  }
  onLiveToggle = () => {
  ...
  getSubMenu = type => {
    if (type === subMenuTye.bg) {
      ...
      return <FontSizeContro onFontSizeChange={this.onFontSizeChange} />;
    }
  };
  onOpenMenuPanel = () => {
    this.setState({
    ...
    return (
      <View style={[this.state.readStyle.bgStyles, {flex: 1}]}>
        <Modal
          ...
          <View>
            ...
            <View style={styles.menuContent}>
              ...
              />
              <ItemMenu
                onPress={this.onLiveToggle}
                icon={this.state.hasLive ? 'heart' : 'heart-o'}
                title={'喜欢'}
              />
            </View>
          </View>
        ...
        <NavigtionBar
          rightButton={
            <TouchableOpacity
              onPress={this.getArticle.bind(
                this,
                DaoArticle.GetArtilceMethod.Random,
              )}>
